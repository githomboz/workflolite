...
class IPN_Handler
{
  ...
  {
    // Validate
    $this->_valid = $this->validate($post_data);
    if($this->_valid !== TRUE)
      return FALSE;
   ...
   */
  protected function validate(array $ipn_post_data)
  {
    // Choose url
    if(array_key_exists('test_ipn', $ipn_post_data) && 1 === (int) $ipn_post_data['test_ipn'])
      $this->_url = self::paypal_sandbox_url;
    else
      $this->_url = self::paypal_url;
    ...
    (
      ...
      CURLOPT_POST => TRUE,
      CURLOPT_POSTFIELDS => http_build_query(array_merge(array('cmd' => '_notify-validate'), $ipn_post_data)),
    ...
    $this->_response = curl_exec($request);
    $status   = curl_getinfo($request, CURLINFO_HTTP_CODE);

    // Close connection
    curl_close($request);

    if($status == 200 && strcmp ($this->_response, "VERIFIED") == 0)
      return TRUE;
