...
class WC_API_Client {
  ...
  public function update_order( $order_id, $data = array() ) {
    return $this->_make_api_call( 'orders/' . $order_id, array('order' => $data), 'POST' ); // Works for v2
    return $this->_make_api_call( 'orders/' . $order_id, $data, 'POST' ); // Works for v1
  }
  ...
  private function _make_api_call( $endpoint, $params = array(), $method = 'GET' ) {
    ...
    //var_dump($this->_api_url.$endpoint.$paramString);
